...
| key | SelectControl choices | default | applies to |
|---|---|---|---|
| `color_scheme` | Default / DHIS2 / Warm / Cool / Earth / Pastel | Default | all chart types |
| `show_legend` | Off / Bottom / Top / Right | Bottom | all with multi-series |
| `show_values` | Off / On | Off | bar, line |
| `y_format` | Default / 1,234 / 1.2K / % | Default | bar, line, combined |
| `x_rotation` | 0 / 45 / 90 | 0 | bar, line, combined |
| `log_scale` | Off / On | Off | bar, line, combined |
| `x_title` | text input | "" | bar, line, combined |
| `y_title` | text input | "" | bar, line, combined |

### Plugin-specific options (define per plugin)

Document each option with: key, type, choices/range, default, behavior description.
