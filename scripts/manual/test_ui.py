...
class TestNonUIRegression(unittest.TestCase):
    ...
    def test_16_4_area_map_generates_html(self):
        """16.4 — AreaMapPlugin generates valid HTML with required JS hooks."""
        from charts.plugins.area_map import AreaMapPlugin
        from charts.fixed_templates import generate_preview_page

        cfg = {
            "plugin_id":      "area_map",
            "template_id":    "area_map",
            "title":          "Test Area Map",
            "col_width":      6,
            "chart_color":    "#3182bd",
            "metrics":        [{"uid": "TstDE0001", "name": "Cases", "type": "aggregate", "agg": "SUM"}],
            "dimensions":     {"time_grain": "Monthly"},
            "plugin_options": {"ou_level": "Level 2", "color_scheme": "Blues", "show_labels": "Hide"},
            "custom_options": {},
            "de_uid":         "TstDE0001",
            "de_sources":     [{"uid": "TstDE0001", "name": "Cases", "type": "aggregate"}],
        }
        # build_js must not raise
        js = AreaMapPlugin.build_js(1, cfg)
        self.assertIn("initChart1", js)
        self.assertIn("AreaMap", js)
        self.assertIn("geoFeatures", js)

        # full page must be valid HTML
        html = generate_preview_page(cfg, title="Test Area Map")
        self.assertIn("<html", html.lower())
        self.assertGreater(len(html), 200)

        # No metric → error JS, not exception
        cfg_empty = dict(cfg, metrics=[])
        js_err = AreaMapPlugin.build_js(1, cfg_empty)
        self.assertIn("initChart1", js_err)

    def test_16_5_point_map_generates_html(self):
        """16.5 — PointMapPlugin generates valid HTML with required JS hooks."""
        from charts.plugins.point_map import PointMapPlugin
        from charts.fixed_templates import generate_preview_page

        cfg = {
            "plugin_id":      "point_map",
            "template_id":    "point_map",
            "title":          "Test Point Map",
            "col_width":      6,
            "chart_color":    "#e74c3c",
            "metrics":        [{"uid": "TstDE0002", "name": "Facilities", "type": "aggregate", "agg": "COUNT"}],
            "dimensions":     {"time_grain": "Monthly"},
            "plugin_options": {"ou_level": "Level 4", "point_scale": "Medium", "point_color": "Red"},
            "custom_options": {},
            "de_uid":         "TstDE0002",
            "de_sources":     [{"uid": "TstDE0002", "name": "Facilities", "type": "aggregate"}],
        }
        js = PointMapPlugin.build_js(2, cfg)
        self.assertIn("initChart2", js)
        self.assertIn("PointMap", js)
        self.assertIn("geoFeatures", js)
        self.assertIn("circleMarker", js)

        html = generate_preview_page(cfg, title="Test Point Map")
        self.assertIn("<html", html.lower())
        self.assertGreater(len(html), 200)

        # Tracker numeric source
        cfg_tracker = dict(cfg, metrics=[{
            "uid": "TstDE0003", "name": "Cases", "type": "tracker_numeric",
            "agg": "SUM", "prog_uid": "progAAA", "stage_uid": "stgBBB",
        }])
        js_tracker = PointMapPlugin.build_js(3, cfg_tracker)
        self.assertIn("analytics/events/aggregate/progAAA", js_tracker)

        # No metric → error JS, not exception
        js_err = PointMapPlugin.build_js(1, dict(cfg, metrics=[]))
        self.assertIn("initChart1", js_err)
